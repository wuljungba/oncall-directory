        ...
        <div className="lg:col-span-2 bg-gray-900 border border-gray-800 rounded-xl p-5">
          {selectedTree ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 text-gray-500">
              ...
              <p className="text-xs mt-1">to view and manage the escalation path</p>
            </div>
          )}
        </div>
      </div>
...
function TreeFormModal({
  ...
  const [treeType, setTreeType] = useState(initial?.treeType || 'department')
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError(null)
    try {
      await onSave({ name: name.trim(), treeType: treeType as any, fallbackProcedure: fallback.trim() || undefined })
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          ...
          <div>
            ...
            <input type="text" required value={name} onChange={(e) => setName(e.target.value)}
              placeholder="e.g., ER Emergency Tree"
          ...
          <div>
            ...
            <textarea value={fallback} onChange={(e) => setFallback(e.target.value)} rows={2}
              placeholder="What to do if no one answers at the last node"
...
function AddNodeModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={onClose}>
      <div className="bg-gray-900 border border-gray-800 rounded-xl w-full max-w-md mx-4" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-800">
