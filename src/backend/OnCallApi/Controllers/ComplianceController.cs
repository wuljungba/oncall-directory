...
public class ComplianceController : ControllerBase
{
    ...
    public async Task<ActionResult<List<DutyHourViolation>>> CheckEmployee(Guid employeeId, [FromQuery] DateTime? from, [FromQuery] DateTime? to, [FromQuery] int? departmentId)
    {
        return await _dutyHourService.CheckComplianceAsync(employeeId, from, to, departmentId);
    }

    [HttpGet("check")]
    public async Task<ActionResult<List<DutyHourViolation>>> CheckAll([FromQuery] DateTime? from, [FromQuery] DateTime? to, [FromQuery] int? departmentId)
    {
        return await _dutyHourService.CheckAllComplianceAsync(from, to, departmentId);
    }
